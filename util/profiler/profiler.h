...
public:
    ...
    void generateReport(const std::string& filename = "profile_report.json") {
        std::lock_guard<std::mutex> lock(m_mutex);
...
// 测量宏
#define PROFILE_SCOPE(name) Profiler::Timer CONCAT(timer_, __LINE__)(Profiler::getInstance(), name)
